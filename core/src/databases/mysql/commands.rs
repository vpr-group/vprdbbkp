...
use super::{installer::MySQLInstaller, version::MySQLVersion};
...
impl CommandBuilder {
    ...
    pub async fn build_base_cmd(&self, bin_name: &str) -> Result<Command> {
        ...
        if !path.exists() {
            let installer = MySQLInstaller::new(self.version);
            match installer.install(self.cache_dir.clone()).await {
                Ok(path) => {
                    if path.exists() {
                        println!(
                            "Successfully installed MySQL {} to {}",
                            self.version,
                            path.display()
                        )
                    } else {
                        return Err(anyhow!("Binary for MySQL {} not found", self.version));
                    }
                }
                Err(error) => return Err(anyhow!("Failed to install MySQL {}", error)),
            };
        }
    ...
    pub async fn build_connection_check_command(&self) -> Result<Command> {
        let mut cmd = self.build_base_cmd("MySQL").await?;
    ...
    pub async fn build_dump_command(&self) -> Result<Command> {
        let mut cmd = self.build_base_cmd("MySQL-dump").await?;
